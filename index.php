...
if (isset($_POST['login'])){
	...
	if($domain=="admin"){		
		...
		if($cek){
			...
			$_SESSION['waktu']=gmdate("Y-m-d H:i:s" , time()+60*60*7);
	...
	if($domain=="guru"){
		$query=mysqli_query($koneksi,"select * from data_guru where username='$username' and password='$password'");
		$cek=mysqli_num_rows($query);
		$row=mysqli_fetch_array($query);
		$id_guru=$row['id_guru'];
	...
	if($domain=="siswa"){
		$query=mysqli_query($koneksi, "select * from data_siswa where username='$username' and password='$password'");
		$cek=mysqli_num_rows($query);
		$row=mysqli_fetch_array($query);
