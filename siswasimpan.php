...
$adadihati=mysqli_query($koneksi,$cekricek) or die(mysqli_error());
...
else if(mysqli_num_rows($adadihati)>0) {
	echo '<script>
...

{
	...
	$rs=mysqli_query($koneksi,"Insert into data_siswa (`nama_siswa`,`nis`,`nisn`,`kota`,`tgl`,`bln`,`thn`,`kelamin`,`agama`,`status`,`anakke`,`alamat_siswa`,`telpon_siswa`,`sekolahasal`,`kelas`,`pd_tgl`,`pd_bln`,`pd_thn`,`ayah`,`ibu`,`alamatortu`,`hportu`,`pekerjaanayah`,`pekerjaanibu`,`wali`,`alamatwali`,`hpwali`,`pekerjaanwali`,`gambar`,`username`,`password`) values 
