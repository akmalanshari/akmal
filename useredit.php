...
{
	$rs=mysqli_query($koneksi, "Select * from user_admin where sha1(username)='".$_GET['id_admin']."'");
...
<center>
  <table width="367" height="270" border="0" align="center" cellpadding="0" cellspacing="0">
    ...
    <tr>
      ...
      <td><input type="text" name="password" id="password" class="inp-form" value="<?php echo $row['password']; ?>"></td>
    </tr>
    <tr>
      <td height="63">&nbsp; &nbsp; &nbsp;</td>
...
{
	$rs=mysqli_query($koneksi, "Update user_admin SET nama_admin='".$_POST['nama_admin']."',username='".$_POST['username']."',password='".md5($_POST['password'])."' Where sha1(username)='".$_GET['id_admin']."'");
	if($rs)
	{
		echo "<script>window.location='?page=user'</script>";
	}
