...
<h2>
<?php
$hsl= mysqli_query($koneksi,'select * from data_siswa');
	  
	  $num_rows = mysqli_num_rows($hsl);
...
<table width="93%" border="0" cellspacing="0" cellpadding="0" id="product-table">
 <tr bgcolor="#99CC33">
  ...
  <?php
  $rw=mysqli_query($koneksi,"Select * from data_siswa order by nama_siswa asc");
...
{
	$ids=$_GET['id_siswa'];
	$ff=mysqli_query($koneksi,"Delete from data_siswa Where sha1(id_siswa)='".$ids."'");
	if($ff)
	{
