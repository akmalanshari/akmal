...
<table width="68%" border="0" cellspacing="0" cellpadding="0" id="product-table" >
  ...
  <?php
  $rw=mysqli_query($koneksi,"Select * from user_admin");
  $no=0;
 ...
 
{
	...
	$rs=mysqli_query($koneksi, "Insert into user_admin (`nama_admin`,`username`,`password`) values ('".$_POST['nama_admin']."','".$_POST['username']."','".$_POST['password']."')") or die(mysql_error());
	if($rs)
	{
...
{
	$ids=$_GET['id_admin'];
	$ff=mysqli_query($koneksi, "Delete from user_admin Where sha1(username)='".$ids."'");
	if($ff)
	{
		echo "<script>window.location='?page=user'</script>";
	}
