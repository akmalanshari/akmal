...
{
	$rs=mysqli_query($koneksi,"Select * from setup_kelas where sha1(nama_kelas)='".$_GET['id_kelas']."'");
...
if(isset($_GET['edit']))
{
	...
	$rs=mysqli_query($koneksi,"update setup_kelas set nama_kelas='".$_POST['nama_kelas']."',jurusan='".$_POST['jurusan']."' Where sha1(nama_kelas)='".$_GET['id_kelas']."'");
	if($rs)
	{
		echo "<script>window.location='?page=kelastampil'</script>";
	}
